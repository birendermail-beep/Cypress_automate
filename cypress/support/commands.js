...
Cypress.Commands.add('checkAscSort', (tbodySelector, colSelector) => {
	cy.get(tbodySelector).within($tbody => {
		let firstStr
		cy.get('tr:nth-child(1) > td:nth-child(' + colSelector + ')')
			.invoke('text')
			.then(text => {
				firstStr = text.trim()
			})
		cy.get('tr').each((arg, indexT, arr) => {
			cy.get(
				'tr:nth-child(' + (indexT + 1) + ') > td:nth-child(' + colSelector + ')'
			)
				.invoke('text')
				.then(text => {
	...
	cy.get('div[intro-id="item_info"]').then($text => {
		if (
			$text
				.text()
				.trim()
				.charAt($text.text().trim().indexOf(' ') - 1) == 1
		) {
...
Cypress.Commands.add('cardNavigation', () => {
	...
	cy.get('div.flipbox:contains("Front: click to flip")')
		.eq(0)
		.click()
		.then(() => {
...
Cypress.Commands.add('openDashboard', () => {
	...
	cy.fixture('global').then(data => {
		cy.visit(
			data.url +
				'/?func=load_course&course=LO-Aplus-complete&class_code=' +
				data.class_code[6]
		)
	})
})
