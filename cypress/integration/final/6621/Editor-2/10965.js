...
@test_steps:
^OpenTranscript directly throgh the video player tag if adding the transcript by add transcript button(If guid is valid)
-open editor
-Add video player tag in section, question or fact.(Any of them).
-In url add the asset link
-click add transcript, if Transcript found then it will add the guid otherwise a modal will open.
-if modal open then add the mandatory details in the modal and then click add.
-If all the details in the modal is correct then on clicking add button, the transcript id will be added automatically in transcript field, otherwise  error message will show.
-If transcript Id is added then, Edit button will be enabled.
-On clicking Edit button, Transcript guid will be open in the new tab if guid is valid

^OpenTranscript directly throgh the video player tag by typing the valid transcript id
-open editor
-Add video player tag in section, question or fact.(Any of them).
-In the transcript id add 5 characters valid guid.
-Edit button will be enabled.
-Click on edit button, transcript id will be open in new tab.

^OpenTranscript directly throgh the video player tag by typing the invalid transcript id
-open editor
-Add video player tag in section, question or fact.(Any of them).
-In the transcript id , type  invalid guid.
-Edit button will be enabled.
-Click on edit button, as guid is invalid it will open the editor.

@test_data: n/a
@result: Editor will be open
*/
