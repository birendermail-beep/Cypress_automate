...
@phase_id: 
@story_id: 10948
@story_name: Load simulation player
@path: final/6621
@test_case_name: Load simulation player.js
@description: 
@test_steps:
^Load simulation player
...
describe("embed (knowledge check) testing", function() {
    it('embed and player tag (knowledge check) for Lab with simulation ', function() {
        ...
        cy.get('#type').select('Simulation', { force: true })
        cy.get('[placeholder="Enter the title"]').type('Test', { force: true })
        cy.get('#asset').clear().type('03t1L', { force: true })
