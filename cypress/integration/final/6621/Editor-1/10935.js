...
@phase_id: 
@story_id: 10935
@story_name: Match List (Runtime arrow)
@path: final/6621
@test_case_name: Match List (Runtime arrow).js
@description: 
@test_steps:
^open module
-Open url https://www.ucertify.com/editor/?action=new
-Click search and type match list
-Open Match List (Runtime arrow)
...
describe("Linux terminal", function() {
    beforeEach("Match List (Runtime arrow)", function() {
        cy.fixture('global').then(data => {
            ...
            cy.get('.icomoon-shuffle').click({ force: true })
        })
    })
    ...
    it('Select Answer With Enter Button', function() {
        cy.fixture('global').then(data => {
            //cy.visit(data.url)
            ...
            EditorPage.visitEditor(data.url);
            cy.wait(5000);
            cy.get('[data-cy=item34]').click({force:true});
            cy.wait(3000);
            cy.get('#edi_tabs > :nth-child(2) > a').click({force: true});
            cy.get('[data-cy=id2]').type('{enter}');
            cy.get('[data-cy=idC]').type('{enter}');
            cy.get('[data-cy=id3]').type('{enter}');
            cy.get('[data-cy=idA]').type('{enter}');
            cy.get('[data-cy=id1]').type('{enter}');
            cy.get('[data-cy=idB]').type('{enter}');
            cy.get('[data-cy=id4]').type('{enter}');
