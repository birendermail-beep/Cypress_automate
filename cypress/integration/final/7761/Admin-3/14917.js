...
@phase_id: 10306
@story_id: 14917
@story_name: email_stop_tab
@path: final/7761
@test_case_name: email_stop_tab.js
@description:
@test_steps:
...
- Click on Email Stop tab
- Select Bounced option from the Email stop list
- Enter message in comment box
- Click on save comments button
- Click on Save button to save the records

@result:
- Email stop will be active & comment will be saved.
 */

import { Navbar, login_username, login_password, LoginPage, AdminArea, CreateArea } from '../../../../page-objects/pages/index' 
describe('Communication area Email Stop tab', function() {
    it('Communication area Email Stop tab', function() {
        cy.fixture('global').then(data => {
            ...
            cy.get('[data-cy="email_stop_tab"]').click({force: true})
            cy.get('[data-cy="email_stop_status_select"]').select('-1', {force: true})
            cy.get('[data-cy="email_stop_comnt_txt"]').type('Testing 3 dot Email Stop Bouced', {force: true})
