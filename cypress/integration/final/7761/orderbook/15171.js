...
@phase_id: 
@story_id: 15171
@story_name: invoice print send email complete
@path: final/Dump_Test_Automation
@test_case_name: invoice_print_send_email_complete.js
@description: Go to inside sales and KPI
@test_steps: 
...
describe('Orderbook Area', function() {
    ...
    it('Successfully Send Email', function() {
        cy.fixture('global').then(data => {
            ...
            cy.get('.token > .close').click({ force: true });
            cy.get('#email_to-tokenfield').clear({ force: true }).type(login_username + '{enter}', { force: true }).blur();
            cy.get('#email_subject').clear({ force: true }).type('Regarding for testing', { force: true });
            cy.get('#invoice_submit').click({ force: true });
        })
    })
})